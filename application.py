...
def buy():
    ...
    if request.method == "POST":
        ...
        if rows[0]["cash"] > float(amount)*float(stock["price"]):
            ...
            db.execute("INSERT INTO history (id, symbol, shares, price, transacted) VALUES(:id, :symbol, :shares, :price, :tran)", id= session['user_id'], symbol=request.form.get("symbol"), shares=request.form.get("shares"), price=stock["price"],tran=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            if not unique:
...
def sell():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO history (id, symbol, shares, price, transacted) VALUES(:id, :symbol, :shares, :price, :tran)", id= session['user_id'], symbol=request.form.get("symbol"), shares=-int(request.form.get("shares")), price=stock["price"], tran=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
